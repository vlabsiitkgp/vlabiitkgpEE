...
Str +="<option value=" + ratioc1c2[5] + ">"+ ratioc1c2[5] +"</option></select>";
	...
	/*for(var l=0;l<6;l++)
	{
document.getElementById('selratio').options[document.getElementById('selratio').options.length] = new Option(ratioc1c2[l],ratioc1c2[l]);
	}*/
...
function calcriplecap1()
{
	...
	{		
		...
		document.getElementById('txt4').disabled=true;*/
			document.getElementById('td5').style.visibility="visible";
			multerm1= 2*f1;//alert("multerm12*f:"+multerm1);
			divterm1= I1/multerm1;//alert("divterm1I/2f:"+divterm1); 
...
function displaygraphcap1()
{
	...
	var myData = new Array();
	for(var j=0;j<riplesel3len;j++){myData[j]= [cap1arr[j],riple3arr[j]];}
	alert("myData:"+myData); 
	myData.sort(sort1); 
	function sort1(a,b)
	{
	...
	myChart.setFlagShape('square');
	myChart.setAxisNameY('Ripple(in mV)');
	myChart.setAxisNameX('Ratio of C1 to C2(in µF)');
	myChart.setAxisNameFontSizeX(15);
	myChart.setAxisNameFontSizeY(15);
	myChart.setAxisNameFontFamilyX('BookmanOldStyle');
	myChart.setAxisNameFontFamilyY('BookmanOldStyle');
...
{	
	...
	/*else if(chooseratio==0)
	{
document.getElementById('divtxt6').innerHTML = "<div class= style2>"+msg5+"</div>";
...
{	
		...
		//C2 = parseFloat(document.getElementById('txt4').value);
		C11 = C11.toFixed(2);//C2= C2.toFixed(2);
